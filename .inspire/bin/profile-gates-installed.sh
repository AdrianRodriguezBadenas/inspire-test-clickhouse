...
# the spec for a project's mechanical enforcement, and until now nothing compared that spec
# to reality. Measured on this project the day the rule was written: the profile promised
# three plugin sets and a coverage floor, the config had none of them, and every run of
# every other validator passed. Prose is not enforcement — the failure class of
# 20260730_a-rule-is-not-in-force-until-loaded-or-checked, one level up: there the rule was
# written and unchecked, here the *checker* was written and uninstalled.
#
