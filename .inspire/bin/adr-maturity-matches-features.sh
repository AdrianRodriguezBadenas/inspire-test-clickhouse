...
#
# Several features legitimately rest on one decision — here two features shared one
# transport ADR — and the fix for a stale ADR is a single `promote` however many features
# cite it. Reporting per pair emitted the same file twice with two nearly identical
# messages, which is the shape people learn to skim. So: collect claimants first, report
